...
#Accept request test
  def test_accept_request(test_app, setup_users):
     student, staff, _ = setup_users
     with test_app.app_context():
       # Ensure there are no pending requests before starting
       db.session.query(Request).delete()
       db.session.commit()
       ...
       assert req.status == "approved" # Status should be 'approved' based on Request.accept() method
  ...
  def test_accept_updates_student_record(test_app, setup_users):
      ...
      with test_app.app_context():
          # Refresh student_record to get current state (total_hours = 0.0)
          ...
          # Refresh the student record to reflect changes from req.accept()
          db.session.refresh(student_record) 

          assert student_record.total_hours == starting_hours + hours_to_add
          # Check that LoggedHours entry was also created
  ...
  def test_denied_does_not_update_hours(test_app, setup_users):
      ...
      with test_app.app_context():
          ...
          assert student_record.total_hours == starting_hours # Should still be 0.0 (or the initial starting_hours)
          # Check that no LoggedHours entry was created for this request
